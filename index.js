...
//first scene is the peace scene
const firstScene = new Scene('Life had been good for quite some time.', 'peace-scene', 'Peace was something that hard been earned 1000 years ago during the great battle of Eiikarti. The war that had raged on before this interlude had lasted for 3000 years. The mad kings conquest had wiped out many races completely, while converting their lands into hellscapes.');
firstScene.setScene(h1SceneDesc, divSceneImage, pSceneDesc)
//this is the scene that shows dragons in the sky.
const secondScene = new Scene('Monsters no like other.', 'dragon-scene', 'The mad king had obtained an artifact from a magical creature that was sacred. Dragons were never evil creatures, but with this horn he could summon one per day, to do his bidding. The mad king was thought to have died because he disappeared for centuries. Dragons were believed to be encapsulated in the plane that the horn summons them from. Their own magical world that kept them away from this one.')
const buttonNextScene = document.querySelector('.button-further-story');
const thirdScene = new Scene('The giants were the first to see the fiece beast.', 'giant-scene', 'The giants were not worried when they saw one dragon on the horizon. They were more in awe, as a single dragon attacking a nation of giants would be suicide. Regardless this was strange and a council meeting was called upon the existing races to speak how a dragon has appeared 1000 after the great war.')
const fourthScene = new Scene('The Council', 'council-scene', 'As the council reached a nervous decision to track the dragon to where it is staying to see if there are more, the earth began to quake. The wind had been strong for days now but they had thought this was only a storm.')
const fifthScene = new Scene('The skies were full of dragons', 'sky-full-dragon', 'The council rushed outside to find the sky full of dragons and other types of weird monster. There was still hope if the dragon horn was used in time to send them back to where they came from.')
const sixthScene =  new Scene('Hope had prevailed', 'dragon-horn-scene','Luckily the dragon horn was secured and blown in time to send three-fourths of the dragons back to their realm. The chance of victory was now about even. This meant it was time to gear up and destroy the wizard before he has enough time to create another army of foul beasts.')
const seventhScene = new Scene('The War for Peace', 'peace-war', 'The next great war for peace is marching forward unyeilding to fatigue. We must take arms and sway the winds of war in our favor. The battle begins.')

scenesArray = [firstScene,secondScene,thirdScene, fourthScene, fifthScene, sixthScene, seventhScene];




buttonNextScene.addEventListener('click', ()=>{
    if(scenesArray.length != 0){
        scenesArray[0].setScene(h1SceneDesc, divSceneImage, pSceneDesc)
        scenesArray.splice(0,1)
        if(scenesArray.length != 0){
            scenesArray[0].setScene(h1SceneDesc, divSceneImage, pSceneDesc)
        }
    }
    //End of scenes.
    if(scenesArray.length === 0){    
document.querySelector('.instructions-to-game-page').classList.remove('hide')
